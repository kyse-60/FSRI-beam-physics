...
# one solved case in full: (a) the input tip force, (b) the response field
# as a heat map, (c) the same field as a plain 3D surface
case = show[0]
sample = fields[case] / np.abs(fields[case]).max()         # (n_time, n_xi)

figure = plt.figure(figsize=(15, 4.2))
ax_in = figure.add_subplot(1, 3, 1)
ax_in.plot(tau, loads[case], color="black", lw=1.5)
ax_in.set(xlabel="normalized time $\\tau$", ylabel="tip force $f(\\tau)$",
          title=f"(a) input: tip force of case {case}")

ax_map = figure.add_subplot(1, 3, 2)
image = ax_map.pcolormesh(tau, xi, sample.T, cmap="RdBu_r",
                          vmin=-1.0, vmax=1.0)
figure.colorbar(image, ax=ax_map, label="$\\eta/\\eta_{\\max}$")
ax_map.set(xlabel="normalized time $\\tau$",
           ylabel="position $\\xi$ (0 = clamp, 1 = tip)",
           title="(b) output field: heat map")

ax_3d = figure.add_subplot(1, 3, 3, projection="3d")
tau_grid, xi_grid = np.meshgrid(tau, xi)           # both (n_xi, n_time)
ax_3d.plot_surface(tau_grid, xi_grid, sample.T, color="lightsteelblue",
                   rstride=1, cstride=2, linewidth=0, antialiased=True)
ax_3d.set(xlabel="time $\\tau$", ylabel="position $\\xi$",
          zlabel="$\\eta/\\eta_{\\max}$", title="(c) output field: 3D surface")
ax_3d.view_init(elev=25, azim=-60)                 # a clear reading angle
figure.tight_layout()
